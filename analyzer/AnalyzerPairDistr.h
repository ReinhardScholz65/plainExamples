...
#ifndef LEMONADE_ANALYZER_PAIR_DISTR_H
#define LEMONADE_ANALYZER_PAIR_DISTR_H
...
/*************************************************************************
 * definition of AnalyzerPairDistr class
 * ***********************************************************************/

/**
 * @file
 *
 * @class AnalyzerPairDistr
 *
 * @brief Analyzer for evaluating the pair distribution g2 as a function of squared lattice vector
 *
 ...
 * also write a new analyzer, inheriting from AnalyzerPairDistr, and
 * overwriting the initialize function.
 */
template < class IngredientsType > class AnalyzerPairDistr : public AbstractAnalyzer
{
...
private:
	...
	std::vector< std::vector<double> > PairDistrTimeSeries;
...
public:
	...
	//! constructor
	AnalyzerPairDistr(const IngredientsType& ing,
			  std::string filename="PairDistrTimeSeries.dat");

	//! destructor. does nothing
	virtual ~AnalyzerPairDistr(){}
...
AnalyzerPairDistr<IngredientsType>::AnalyzerPairDistr(
	const IngredientsType& ing,
	std::string filename)
:ingredients(ing)
,PairDistrTimeSeries(4,std::vector<double>(0))
,bufferSize(100)
,outputFile(filename)
 ...
 * */
template< class IngredientsType >
void AnalyzerPairDistr<IngredientsType>::initialize()
{
...
std::cout << "from AnalyzerPairDistr " << groups.size() << " " << ingredients.getMolecules().size() << std::endl;
}
 ...
 * */
template< class IngredientsType >
bool AnalyzerPairDistr<IngredientsType>::execute()
{
	...
	PairDistrTimeSeries[0].push_back(Rg2Components.getX());
	PairDistrTimeSeries[1].push_back(Rg2Components.getY());
	PairDistrTimeSeries[2].push_back(Rg2Components.getZ());
	PairDistrTimeSeries[3].push_back(Rg2Components.getX()+Rg2Components.getY()+Rg2Components.getZ());
...
template<class IngredientsType>
void AnalyzerPairDistr<IngredientsType>::cleanup()
{
	std::cout<<"AnalyzerPairDistr::cleanup()...";
 ...
 * */
template<class IngredientsType>
void AnalyzerPairDistr<IngredientsType>::dumpTimeSeries()
{
	//fist make a single vector<vector<double> > for writing the results
	std::vector<std::vector<double> > resultsTimeseries=PairDistrTimeSeries;
	...
	if(isFirstFileDump){
		...
		commentTimeSeries<<"Created by AnalyzerPairDistr\n";
	...
	MCSTimes.resize(0);
	PairDistrTimeSeries.resize(0);
	PairDistrTimeSeries.resize(4,std::vector<double>(0));
}
 ...
 * */
template<class IngredientsType>
VectorDouble3 AnalyzerPairDistr<IngredientsType>::calculateRg2Components(
